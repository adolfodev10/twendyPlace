  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 2xl:grid-cols-4 gap-3 sm:gap-4 lg:gap-6">
      {products.map((product) => {
        const isPartner = 'isPartnerProduct' in product && Boolean((product as any).isPartnerProduct);
        const originalPrice = 'originalPrice' in product ? (product as any).originalPrice : undefined;
        return (
          <div
            key={product.id}
          ...
          >
            ...
            <div className="p-3 sm:p-4">
              ...
              {/* Adicionar badge de "Parceiro" nos produtos */}
              {isPartner && (
                <span className="absolute top-2 right-2 bg-purple-600 text-white text-xs font-bold px-2 py-1 rounded-lg">
                  Parceiro
                </span>
              )}

              {/* No preço, mostrar preço original riscado se for parceiro */}
              {isPartner && originalPrice && originalPrice > product.price && (
                <span className="text-xs text-gray-400 line-through block">
                  Kz {originalPrice.toFixed(2)}
                </span>
              )}
